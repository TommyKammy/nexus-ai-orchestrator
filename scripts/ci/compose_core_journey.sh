...
| (.nodes[] | select(.name=="Validate and Filter") | .parameters.jsCode) = "const crypto = require('crypto'); const input = $input.first().json.body || {}; const tenantId = String(input.tenant_id || '').trim(); const scope = String(input.scope || '').trim(); const text = String(input.text || '').trim(); const facts = Array.isArray(input.facts) ? input.facts : []; const tags = Array.isArray(input.tags) ? input.tags : []; const source = String(input.source || 'unknown'); const metadata = (input.metadata && typeof input.metadata === 'object' && !Array.isArray(input.metadata)) ? input.metadata : {}; if (!tenantId || !scope || !text) { return [{ json: { error: 'Missing required fields: tenant_id, scope, text' } }]; } const requestId = String(input.request_id || `req-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`); const contentHash = crypto.createHash('sha256').update(text, 'utf8').digest('hex'); return [{ json: { tenant_id: tenantId, scope, text, facts: facts.map((f)=>({subject:String(f.subject||'').trim(),predicate:String(f.predicate||'').trim(),object:String(f.object||'').trim(),confidence:Number.isFinite(Number(f.confidence))?Number(f.confidence):1})).filter((f)=>f.subject&&f.predicate&&f.object), tags, source, metadata, content_hash: contentHash, request_id: requestId } }];"
